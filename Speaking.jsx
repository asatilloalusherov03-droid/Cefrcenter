// Speaking.jsx — v18 — Leaderboard + Epic Design + TTS fallback
...
function clientScore(transcript) {
  ...
  // Keep the scorer forgiving for short but meaningful answers.
  const englishCheck = /[a-zA-Z]{2,}/;
  if (!safe) {
    return { fluency: 0, vocabulary: 0, grammar: 0, pronunciation: 0 };
  }
  if (!englishCheck.test(safe)) {
    return { fluency: 6, vocabulary: 5, grammar: 5, pronunciation: 5 };
  }
  if (wc < 4) {
    return { fluency: 10, vocabulary: 9, grammar: 8, pronunciation: 9 };
  }
...
// ─── MAIN SPEAKING ─────────────────────────────────────────────────────────────
export default function Speaking({ user, progress, scores, saveScore, addXP, addCoins, tests = [] }) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  if (view === "list") {
    const doneCount = tests.filter(t => isTestDone(t)).length;
    ...
    return (
      <div style={{ animation: "fadeUp .4s ease" }}>
        <style>{`
          ...
          @keyframes spin{to{transform:rotate(360deg)}}
          @keyframes micPulse{0%,100%{box-shadow:0 0 0 0 rgba(212,83,126,0.4)}70%{box-shadow:0 0 0 10px rgba(212,83,126,0)}}
          .sp-card:hover{transform:translateY(-2px) !important;box-shadow:0 8px 32px rgba(0,0,0,0.3) !important;}
        `}</style>

        {/* Hero Header */}
        <div style={{
          background: "linear-gradient(135deg, rgba(212,83,126,0.12) 0%, rgba(127,119,221,0.06) 100%)",
          border: "1px solid rgba(212,83,126,0.2)",
          borderRadius: 20, padding: "22px 24px", marginBottom: 20,
          position: "relative", overflow: "hidden",
        }}>
          <div style={{ position:"absolute",top:-40,right:-40,width:140,height:140,borderRadius:"50%",background:"radial-gradient(circle,rgba(212,83,126,0.15) 0%,transparent 70%)",pointerEvents:"none" }} />
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <div style={{ fontSize:32, animation:"micPulse 2s ease-in-out infinite" }}>🎤</div>
              <div>
                <h2 style={{ fontSize:22, fontWeight:900, color:"#f0f4ff", margin:0 }}>
                  <span style={{ color:"#D4537E" }}>Speaking</span> Assessment
                </h2>
                <p style={{ color:"#4a5568", fontSize:12, margin:0 }}>AI-powered · Real-time transcript · IELTS-style scoring</p>
              </div>
            </div>
            <div style={{ background:hasBrowser?"rgba(29,158,117,0.1)":"rgba(225,29,72,0.1)", border:`1px solid ${hasBrowser?"rgba(29,158,117,0.3)":"rgba(225,29,72,0.3)"}`, borderRadius:10, padding:"6px 12px", display:"flex", alignItems:"center", gap:6 }}>
              <div style={{ width:8, height:8, borderRadius:"50%", background:hasBrowser?"#1D9E75":"#e11d48", animation:hasBrowser?"micPulse 2s infinite":"none" }} />
              <span style={{ fontSize:11, fontWeight:700, color:hasBrowser?"#1D9E75":"#e11d48" }}>{hasBrowser ? "Mic Ready" : "No Mic API"}</span>
            </div>
          </div>
          <div style={{ display:"flex", gap:10, marginTop:14, flexWrap:"wrap" }}>
            {[{icon:"🎤",val:tests.length,label:"Tests"},{icon:"✅",val:doneCount,label:"Completed",color:doneCount>0?"#D4537E":undefined},{icon:"🧠",val:tests.reduce((s,t)=>s+(t.parts?.length||0),0),label:"Parts"}].map(({icon,val,label,color})=>(
          ...
          {!hasBrowser && (
            <div style={{ marginTop:12, padding:"10px 14px", background:"rgba(225,29,72,0.08)", border:"1px solid rgba(225,29,72,0.2)", borderRadius:10, fontSize:12, color:"#f87171" }}>
              ⚠️ Speech Recognition not supported. Please use <strong>Google Chrome</strong> or <strong>Microsoft Edge</strong>.
            </div>
          )}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:16, alignItems:"start" }}>
          ...
          <div style={{ background:"linear-gradient(180deg,rgba(255,215,0,0.04),rgba(255,255,255,0.01))", border:"1px solid rgba(255,215,0,0.12)", borderRadius:16, padding:"16px 12px", position:"sticky", top:0 }}>
            <SectionLeaderboard section="speaking" currentUser={user} />
          </div>
          ...
          <div style={{ display:"grid", gap:10 }}>
            {tests.length === 0 ? (
              ...
              const completedParts = t.parts.filter(p => isPartDone(t.id, p.id)).length;
              const pct = t.parts.length > 0 ? (completedParts / t.parts.length) * 100 : 0;
              return (
                <div key={t.id} className="sp-card" onClick={() => openTest(t)}
                  style={{
                    background: done ? "linear-gradient(135deg,rgba(212,83,126,0.07),rgba(212,83,126,0.02))" : "rgba(255,255,255,0.02)",
                    border:`1px solid ${done?"rgba(212,83,126,0.25)":"rgba(255,255,255,0.06)"}`,
                    borderRadius:14, padding:"14px 16px", cursor:"pointer",
                    transition:"all .2s ease", position:"relative", overflow:"hidden",
                    animation:`slideUp .4s ease ${ti*0.05}s both`,
                  }}>
                  {done && <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#D4537E,transparent)"}} />}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:12, flex:1, minWidth:0 }}>
                      <div style={{ width:44, height:44, borderRadius:14, flexShrink:0, background:done?"rgba(212,83,126,0.2)":"rgba(255,255,255,0.04)", border:`2px solid ${done?"rgba(212,83,126,0.5)":"rgba(255,255,255,0.08)"}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:done?16:18 }}>
                        {done ? "✅" : "🎤"}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:3 }}>
                          <span style={{ fontSize:13, fontWeight:700, color:"#f0f4ff", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.title}</span>
                          {done && <span style={{ fontSize:9, background:"rgba(29,158,117,0.15)", color:"#1D9E75", padding:"1px 6px", borderRadius:20, fontWeight:800, flexShrink:0 }}>✓ Done</span>}
                        </div>
                        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                          <span style={{ fontSize:10, background:"rgba(212,83,126,0.15)", color:"#D4537E", padding:"1px 7px", borderRadius:6, fontWeight:700 }}>{t.level}</span>
                          <span style={{ fontSize:10, color:"#4a5568" }}>🎭 {t.parts?.length||3} parts</span>
                          <span style={{ fontSize:10, color:"#4a5568" }}>⏱️ {t.duration}</span>
                        </div>
                        {pct > 0 && (
                          <div style={{ display:"flex", gap:3, marginTop:5, alignItems:"center" }}>
                            {t.parts?.map((p, i) => (
                              <div key={i} style={{ width:isPartDone(t.id,p.id)?10:8, height:isPartDone(t.id,p.id)?10:8, borderRadius:"50%", background:isPartDone(t.id,p.id)?"#1D9E75":"rgba(255,255,255,0.12)", transition:"all .3s", boxShadow:isPartDone(t.id,p.id)?"0 0 6px #1D9E75":"none" }}/>
                            ))}
                            <span style={{ fontSize:9, color:"#4a5568", marginLeft:2 }}>{completedParts}/{t.parts.length} done</span>
                          </div>
                        )}
                      </div>
                    </div>
                    <div style={{ flexShrink:0, textAlign:"right" }}>
                      {bestScore != null ? (
                        <><div style={{fontSize:15,fontWeight:900,color:"#D4537E"}}>{bestScore}<span style={{fontSize:10,opacity:0.6}}>/75</span></div>
                        <div style={{fontSize:9,color:"#D4537E",fontWeight:700,background:"rgba(212,83,126,0.15)",padding:"1px 6px",borderRadius:4}}>Best</div></>
                  ...
                  {pct > 0 && pct < 100 && (
                    <div style={{ marginTop:10, height:3, borderRadius:2, background:"rgba(255,255,255,0.06)", overflow:"hidden" }}>
                      <div style={{ height:"100%", width:`${pct}%`, background:"linear-gradient(90deg,#D4537E,#7F77DD)", borderRadius:2, transition:"width 1s ease" }} />
                    </div>
                  )}
                </div>
              );
      ...
      {phase === "result" && result && (
        <div style={{ textAlign: "center", animation: "fadeUp .4s ease" }}>
          ...
          <div style={{ display: "grid", gap: 16, textAlign: "left" }}>
            ...
            {result.modelAnswer && (
              <div style={{ background: "rgba(240, 243, 247, 0.05)", padding: 20, borderRadius: 12, border: "1px solid rgba(74,158,255,0.1)" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            <button onClick={() => { stopRecording(); setView("list"); }}
              style={{ flex: 1, padding: "14px", borderRadius: 10, border: "1px solid #fff", background: "none", color: "#8b9bbf", fontWeight: "bold", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              <I n="back" s={16} c="#8b9bbf"/> Leave
            </button>
            <button onClick={nextPart}
              style={{ flex: 2, padding: "14px", borderRadius: 10, background: "#4a9eff", color: "#fff", border: "none", fontWeight: "bold", cursor: "pointer", display: "flex", justifyContent: "center", alignItems: "center", gap: 8 }}>
              {pIdx < selTest.parts.length - 1 ? "Next Part" : "Finish Test"} <I n="next" s={16} c="#fff"/>
            </button>
          </div>
        </div>
      )}

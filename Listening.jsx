...
// ── TTS fallback: reads the part title aloud when audio fails ────────────────
...
function AudioPlayer({ audioUrl, partLabel, partType, onEnded, onError, onPlay, alreadyPlayed, partContent }) {
  ...
  const playTTS = () => {
    // Build text from partContent or partLabel
    const text = partContent || `This is ${partLabel}. Please answer the questions based on the content you hear.`;
    setStatus("playing"); setUsingTTS(true);
    onPlay?.();
    hasPlayedRef.current = true;
    const utt = speakText(text);
    if (utt) {
      ttsRef.current = utt;
      utt.onend = () => { setStatus("ended"); onEnded?.(); };
      // Animate fake progress
      const estDur = Math.max(10, text.split(" ").length * 0.45);
      setDuration(estDur);
      const start = Date.now();
      const tick = () => {
        const elapsed = (Date.now() - start) / 1000;
        setProgress(Math.min(elapsed, estDur));
        if (elapsed < estDur && status !== "ended") rafRef.current = requestAnimationFrame(tick);
      };
      rafRef.current = requestAnimationFrame(tick);
    } else {
      setStatus("ended"); onEnded?.();
    }
  };
  ...
  const play = () => {
    ...
    a.load();
    audioRef.current = a;
    // 8-second timeout → fallback to TTS
    const timeout = setTimeout(() => {
      if (status === "loading") {
        a.pause(); audioRef.current = null;
        setStatus("idle"); hasPlayedRef.current = false;
        playTTS();
      }
    }, 8000);
    a.onloadedmetadata = () => { clearTimeout(timeout); if (a.duration && isFinite(a.duration)) setDuration(a.duration); };
    a.oncanplay = () => { clearTimeout(timeout); if (a.duration && isFinite(a.duration)) setDuration(a.duration); setStatus("playing"); };
    ...
    a.onended = () => { clearTimeout(timeout); setStatus("ended"); setProgress(a.duration || 0); if (rafRef.current) cancelAnimationFrame(rafRef.current); onEnded?.(); };
    a.onerror = () => { clearTimeout(timeout); a.pause(); audioRef.current = null; setStatus("idle"); hasPlayedRef.current = false; playTTS(); };
    a.play().catch(() => { clearTimeout(timeout); setStatus("idle"); hasPlayedRef.current = false; playTTS(); });
  };
  ...
  return (
    <div style={{
      background: isEnded ? "rgba(29,158,117,0.06)" : isPlaying ? `${color}08` : "rgba(255,255,255,0.02)",
      border: `1px solid ${isPlaying ? color+"66" : isEnded ? "#1D9E7540" : "rgba(255,255,255,0.08)"}`,
      borderRadius: 14, padding: "14px 18px", marginBottom: 14,
      transition: "all .3s ease",
      boxShadow: isPlaying ? `0 0 24px ${color}22` : "none",
    }}>
      <style>{`
        @keyframes waveBeat{0%,100%{transform:scaleY(0.25)}50%{transform:scaleY(1)}}
        @keyframes livePulse{0%,100%{opacity:1}50%{opacity:0.3}}
        @keyframes spin{to{transform:rotate(360deg)}}
        @keyframes audioGlow{0%,100%{box-shadow:0 0 8px ${color}33}50%{box-shadow:0 0 24px ${color}66}}
      `}</style>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            background: isEnded ? "rgba(29,158,117,0.15)" : isPlaying ? `${color}22` : `linear-gradient(135deg, ${color}, ${color}cc)`,
            display: "flex", alignItems: "center", justifyContent: "center",
            cursor: (status === "idle" && !alreadyPlayed) ? "pointer" : "not-allowed",
            transition: "all .2s", animation: isPlaying ? `audioGlow 2s ease infinite` : "none",
          }}>
          {isPlaying
            ? <div style={{ display: "flex", gap: 2.5, alignItems: "center", height: 20 }}>
                {[6,14,10,18,8,14,6].map((h, i) => (
                  <div key={i} style={{ width: 3, height: h, borderRadius: 2, background: color, animation: `waveBeat ${0.5+i*0.08}s ${i * 0.09}s ease-in-out infinite`, transformOrigin: "center" }} />
                ))}
              </div>
            : isLoading
              ? <div style={{ width: 18, height: 18, borderRadius: "50%", border: "2.5px solid rgba(255,255,255,0.25)", borderTopColor: "#fff", animation: "spin .7s linear infinite" }} />
              : isEnded
                ? <Ic n="check" s={20} c="#1D9E75" />
                : <Ic n="play" s={18} c="#fff" />}
        </button>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
              ...
              {usingTTS && <span style={{ fontSize: 9, background: "rgba(139,92,246,0.2)", color: "#a78bfa", padding: "1px 6px", borderRadius: 20, fontWeight: 800, border: "1px solid rgba(139,92,246,0.3)", flexShrink: 0 }}>🔊 TTS</span>}
            </div>
            {isPlaying && <span style={{ fontSize: 9, background: `${color}22`, color, padding: "2px 8px", borderRadius: 20, fontWeight: 800, animation: "livePulse 1.4s infinite", border: `1px solid ${color}44`, flexShrink: 0, marginLeft: 8 }}>● LIVE</span>}
          ...
          <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,.08)", overflow: "hidden", position: "relative", marginBottom: 5 }}>
            <div style={{ position: "absolute", top: 0, left: 0, height: "100%", width: isEnded ? "100%" : `${pct}%`, background: isEnded ? "#1D9E75" : `linear-gradient(90deg, ${color}, ${color}cc)`, borderRadius: 3, transition: isPlaying ? "none" : "width .3s", boxShadow: isPlaying ? `0 0 8px ${color}` : "none" }} />
          </div>
      ...
      {status === "idle" && !alreadyPlayed && (
        <div style={{ marginTop: 10, fontSize: 11, color: "#e11d48", display: "flex", gap: 6, background: "rgba(225,29,72,.06)", borderRadius: 8, padding: "7px 12px", border: "0.5px solid rgba(225,29,72,0.15)" }}>
          ...
          <span>Audio plays <strong>only once</strong>. If audio doesn't load, browser TTS will be used.</span>
        </div>
      )}
    </div>
  );
...
export default function Listening({ user, scores, saveScore, addXP, progress, onBack, tests = [] }) {
  ...
  if (view === "list") {
    ...
    const totalQ = tests.reduce((s, t) => s + t.totalQuestions, 0);
    const bestScore = Math.max(0, ...tests.map(t => scores?.[`listening_t${t.id}_overall`] ?? 0));
    ...
    return (
      <div style={{ animation: "fadeUp .4s ease" }}>
        <style>{`
          ...
          @keyframes glow{0%,100%{box-shadow:0 0 20px rgba(29,158,117,0.2)}50%{box-shadow:0 0 40px rgba(29,158,117,0.4)}}
          @keyframes slideUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
          .test-card:hover{transform:translateY(-2px) !important;box-shadow:0 8px 32px rgba(0,0,0,0.3) !important;}
          .start-btn:hover{transform:scale(1.02) !important;filter:brightness(1.1) !important;}
        `}</style>

        {/* ── HERO HEADER ── */}
        <div style={{
          background: "linear-gradient(135deg, rgba(29,158,117,0.12) 0%, rgba(55,138,221,0.06) 100%)",
          border: "1px solid rgba(29,158,117,0.2)",
          borderRadius: 20, padding: "22px 24px", marginBottom: 20,
          position: "relative", overflow: "hidden",
        }}>
          {/* Decorative orb */}
          <div style={{ position:"absolute", top:-40, right:-40, width:140, height:140, borderRadius:"50%", background:"radial-gradient(circle, rgba(29,158,117,0.15) 0%, transparent 70%)", pointerEvents:"none" }} />

          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <div style={{ fontSize: 28, animation: doneCount > 0 ? "glow 2s ease infinite" : "none" }}>🎧</div>
                <div>
                  <h2 style={{ fontSize: 22, fontWeight: 900, color: "#f0f4ff", margin: 0, letterSpacing: "-0.5px" }}>
                    <span style={{ color: "#1D9E75" }}>Listening</span> Practice
                  </h2>
                  <p style={{ color: "#4a5568", fontSize: 12, margin: 0 }}>IELTS-style · 6 parts · Auto-graded · Real audio</p>
                </div>
              </div>
            </div>
            {bestScore > 0 && (
              <div style={{ background: "rgba(29,158,117,0.1)", border: "1px solid rgba(29,158,117,0.3)", borderRadius: 12, padding: "8px 14px", textAlign: "center" }}>
                <div style={{ fontSize: 10, color: "#1D9E75", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 }}>Best Score</div>
                <div style={{ fontSize: 22, fontWeight: 900, color: "#1D9E75" }}>{bestScore}<span style={{ fontSize: 12, opacity: 0.6 }}>/75</span></div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
            {[
              { icon: "🎧", val: tests.length, label: "Tests" },
              { icon: "❓", val: totalQ, label: "Questions" },
              { icon: "✅", val: doneCount, label: "Completed", color: doneCount > 0 ? "#1D9E75" : undefined },
              { icon: "⏱️", val: `~${tests.reduce((s,t)=>s+(t.duration||30),0)}`, label: "Min Total" },
            ].map(({ icon, val, label, color }) => (
              <div key={label} style={{
                background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "8px 14px",
                display: "flex", alignItems: "center", gap: 7, border: "1px solid rgba(255,255,255,0.06)",
              }}>
        ...
        {/* ── TWO-COLUMN LAYOUT: leaderboard + tests ── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 16, alignItems: "start" }}>
          ...
          <div style={{
            ...
            <SectionLeaderboard section="listening" currentUser={user} />
          </div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {tests.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 20px", color: "#4a5568", border: "1px dashed rgba(255,255,255,0.08)", borderRadius: 14 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4, color: "#8b9bbf" }}>No tests available</div>
                <div style={{ fontSize: 12 }}>Backend may be offline. Start the backend server.</div>
              </div>
            ) : tests.map((test, idx) => {
              ...
              const pct        = isDone ? Math.round((savedScore / MAX_SCORE) * 100) : 0;
              ...
              return (
                <div
                  ...
                  style={{
                    background: isDone
                      ? "linear-gradient(135deg, rgba(29,158,117,0.06) 0%, rgba(29,158,117,0.02) 100%)"
                      : "rgba(255,255,255,0.02)",
                    border: `1px solid ${isDone ? "rgba(29,158,117,0.25)" : "rgba(255,255,255,0.06)"}`,
                ...
                >
                  ...
                  {isDone && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, transparent, ${cefrInfo?.color || "#1D9E75"}, transparent)` }} />}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{
                        ...
                        background: isDone ? "rgba(29,158,117,0.2)" : "rgba(255,255,255,0.04)",
                        border: `2px solid ${isDone ? "rgba(29,158,117,0.5)" : "rgba(255,255,255,0.08)"}`,
                        display: "flex", alignItems: "center", justifyContent: "center",
                      }}>
                        {isDone
                          ? <span style={{ fontSize: 16 }}>✅</span>
                          : <span style={{ fontSize: 14, fontWeight: 900, color: "#8b9bbf" }}>{idx + 1}</span>}
                      </div>

                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#f0f4ff", marginBottom: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {test.title}
                        </div>
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          <span style={{ fontSize: 10, background: "rgba(55,138,221,0.15)", color: "#4a9eff", padding: "1px 7px", borderRadius: 6, fontWeight: 700 }}>{test.level}</span>
                          <span style={{ fontSize: 10, color: "#4a5568" }}>📝 {test.totalQuestions}Q</span>
                          <span style={{ fontSize: 10, color: "#4a5568" }}>⏱️ {test.duration}min</span>
                          <span style={{ fontSize: 10, color: "#4a5568" }}>🎵 {test.parts?.length || 6} parts</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                      {cefrInfo ? (
                        <>
                          <div style={{ fontSize: 15, fontWeight: 900, color: cefrInfo.color }}>{savedScore}<span style={{ fontSize: 10, opacity: 0.6 }}>/75</span></div>
                          <div style={{ fontSize: 9, color: cefrInfo.color, fontWeight: 700, background: `${cefrInfo.color}18`, padding: "1px 6px", borderRadius: 4 }}>{cefrInfo.cefr} Band {cefrInfo.band}</div>
                        </>
                      ) : (
                        <div style={{ fontSize: 11, color: "#4a5568", fontWeight: 600 }}>Start →</div>
                      )}
                    </div>
                  </div>
                  ...
                  {isDone && (
                    <div style={{ marginTop: 10, height: 3, borderRadius: 2, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${pct}%`, background: cefrInfo?.color || "#1D9E75", borderRadius: 2, transition: "width 1s ease" }} />
                    </div>
                  )}
                </div>
              );
